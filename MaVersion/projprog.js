...
let mob = new Image(); //image pikachu de l'écran de transition
	mob.src = "images/pika.png";

let titlescreen = new Image(); //image pour l'écran titre
	titlescreen.src = "images/affpoke.png"

let title = new Image(); //image pour le titre de l'écran titre
	title.src = "images/PokemonTitle.png";

let chef = new Image(); //image pour le menu
	chef.src = "images/red.png";

let credits = new Image(); //image pour le menu
	credits.src = "images/credits.png";

// EN PARTIE RAPIDE
let interCombat = new Image();
	interCombat.src = "images/BattleGround.png";

let aquali = new Image();
	aquali.src = "Sprites/vaporeon.png";

let voltali_dos = new Image();
	voltali_dos.src = "Sprites/jolteon_dos.png";
...
let gener = new Audio('songs/PokemonFireRedTitleScreen.mp3')
gener.loop=true;
gener.volume=0.2;

let creditsound = new Audio('songs/PokemonLeague.mp3');
creditsound.volume=0.2;

let audioMainMenu = new Audio('songs/MainMenu.mp3');

let battle = new Audio('songs/CynthiaTheme.mp3');
battle.loop = true;
battle.volume=0.2;
//
    ...
    if (Gamerunning)
    {
        ...
        if(transit)
        {
            ...
            if((posSourisX>400)&(posSourisY>150)&(posSourisY<190)) //si la souris se trouve dans cette zone...
			{
				...
				{
					prtaudio = 0;
					let audio = new Audio('songs/selection.mp3');
					audio.play();
				}
            ...
            if((posSourisX>400)&(posSourisY>220)&(posSourisY<260))
			{
				...
				window.onclick = () => {
					...
					clearInterval(MENU_Interval_ID)
					console.log(MENU_Interval_ID)
					context.drawImage(interCombat,0,0,canvas.width, canvas.height);
					context.drawImage(aquali,510,220,200,200);
					context.drawImage(voltali_dos,60,250,450,450);

					context.drawImage(fight,520,500,100,60);
